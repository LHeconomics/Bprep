...
for file in "$texname"-Final "$texname"-Slides; do
...
[[ -e "$texname-Final".pdf ]] && rm -f "$texname-Final".pdf

echo '' 

if [[ -e "$output_directory/$texname-Final.pdf" ]]; then
    echo "Paper has been compiled to $output_directory/$texname-Final.pdf"
    echo "and copied to ./$texname-Final.pdf"
    cp "$output_directory/$texname-Final.pdf" "./$texname-Final.pdf"
else
    echo "Something went wrong and the paper is not in $output_directory/$texname-Final.pdf"
fi
